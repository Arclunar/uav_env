    ...
    double voltage)
{
  ...
  const Eigen::Vector3d pid_error_accelerations = computeRealPIDErrorAcc(odom, des, param);
...
Eigen::Vector3d Controller::computeRealPIDErrorAcc(
    const Odom_Data_t &odom,
    const Desired_State_t &des,
    const Parameter_t &param)
{
    ...
    static std::chrono::steady_clock::time_point stamp_last; 
    static std::chrono::steady_clock::time_point stamp_now;
    ...
    {
      first_loop = false;
      dt = 1 / param.ctrl_freq_max;
      stamp_last = std::chrono::steady_clock::now();
    }
    else
    {
        stamp_now = std::chrono::steady_clock::now();
        dt = std::chrono::duration<double>(stamp_now - stamp_last).count();
        // std::cout<<"[controller] dt = "<< dt<<std::endl;
        stamp_last = std::chrono::steady_clock::now();
    }


    //Kvi kvd  
    static double prevDerivative_x = 0;
    static double prevDerivative_y = 0;
    static double prevDerivative_z = 0;

    double last_x_vel_error = last_e_v.x();
    double last_y_vel_error = last_e_v.y();
    double last_z_vel_error = last_e_v.z();
    ...
    // x Derivative term
    double derivative_x = (x_vel_error - last_x_vel_error) / dt;
    double x_D_vel_error = 0.5 * prevDerivative_x + 0.5 * derivative_x;
    prevDerivative_x = derivative_x;

    // std::cout<<"[controller] x_D_vel_error : "<<x_D_vel_error<<std::endl;
    // std::cout<<"[controller] D term  : "<< Kvd(0) * x_D_vel_error <<std::endl;

    // x limited integral term
    double x_I_vel_error = int_e_v.x() + x_vel_error * dt;
    x_I_vel_error = std::max(std::min(x_I_vel_error,int_e_v_lim),-int_e_v_lim);
    int_e_v.x() = x_I_vel_error;

    // std::cout<<"[controller] x_I_vel_error  : "<<x_I_vel_error<<std::endl;
    // std::cout<<"[controller] I term  : "<<Kvi(0) * x_I_vel_error<<std::endl;

    acc_error(0) = Kv(0) * x_vel_error  + Kvd(0) * x_D_vel_error + Kvi(0) * x_I_vel_error;
    ...
    // y Derivative term
    double derivative_y = (y_vel_error - last_y_vel_error) / dt;
    double y_D_vel_error = 0.8 * prevDerivative_y + 0.2 * derivative_y;
    prevDerivative_y = derivative_y;
    
    // y limited integral term
    double y_I_vel_error = int_e_v.y() + y_vel_error * dt;
    y_I_vel_error = std::max(std::min(y_I_vel_error,int_e_v_lim),-int_e_v_lim);
    int_e_v.y() = y_I_vel_error;

    acc_error(1) = Kv(1) * y_vel_error + Kvd(1) * y_D_vel_error + Kvi(1) * y_I_vel_error;
    ...
    // z Derivative term
    double derivative_z = (z_vel_error - last_z_vel_error) / dt;
    double z_D_vel_error = 0.8 * prevDerivative_z + 0.2 * derivative_z;
    prevDerivative_z = derivative_z;
    
    // z limited integral term
    double z_I_vel_error = int_e_v.z() + z_vel_error * dt;
    z_I_vel_error = std::max(std::min(z_I_vel_error,int_e_v_lim),-int_e_v_lim);
    int_e_v.z() = z_I_vel_error;

    acc_error(2) = Kv(2) * z_vel_error + Kvd(2) * z_D_vel_error + Kvi(2) * z_I_vel_error;
